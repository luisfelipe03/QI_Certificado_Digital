...
import { FormPFProps, FormPFSchema, FormPFSchemaValidation } from './FormPFSchema';
import { useCertificadoPFService } from '@/resources/certificado-pf/certificado-pf.service';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faEye, faEyeSlash } from '@fortawesome/free-solid-svg-icons';

interface inputPFModalProps {
    open: boolean;
    setOpen: (open: boolean) => void;
}

interface dataInputPF {
    file: File;
    password: string;
}

export const InputPFModal: React.FC<inputPFModalProps> = ({ open = false, setOpen }: inputPFModalProps) => {
    ...
    const formik = useFormik<FormPFProps>({
        initialValues: FormPFSchema,
        validationSchema: FormPFSchemaValidation,
        onSubmit: handleSubmit
    });

    async function handleSubmit(dados: FormPFProps) {
       setLoading(true);
       const formData = new FormData();
       formData.append('file', dados.certificado);
       ...
       notification.notify('Certificado cadastrado com sucesso', 'success');
    };
                        ...
                        <section className='flex flex-col items-center justify-center my-5'>
                            ...
                            <form onSubmit={formik.handleSubmit}>
                                <div className='mt-4 grid grid-cols-1'>
                                    ...
                                    <div className='mt-2 flex justify-center rounded-lg border border-dashed border-gray-900/25 px-6 py-10'>
                                        <div className='text-center'>
                                            <RenderIf condition={!filePreview}>
                                            ...
                                            <div className='mt-3 flex text-sm leading-6 text-gray-600'>
                                                <label className='relative cursor-pointer rounded-md bg-white font-semibold text-indigo-600'>
                                                    ...
                                                    <RenderIf condition={!!filePreview}>
                                                        <img src={filePreview} width={250}  className='rouded-md' />
                                                    </RenderIf>
                                ...
                                <div className='mt-5 mb-5 flex items-center justify-center gap-x-6'>
                                    ...
                                    <Button onClick={handleCloseModal} style='bg-red-500 hover:bg-red-600' label='Cancelar' type='button' />
                                </div>
                            </form>
                        </section>
                        </div>
