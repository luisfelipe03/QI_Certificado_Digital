...
export default function LucroPresumidoPage() {
    ...
    async function loadCertificados(query: string = '', type: 'razao' | 'cnpj' = 'razao') {
        ...
        if (query === '') {
            const result = await useService.getAllByTipo('LP', page, limit);
            data = result.data;
            setTotal(result.total);
            setPaginacao(true);
        } else if (type === 'razao') {
            const result = await useService.getByRazaoAndTipo(query, 'LP');
            data = Array.isArray(result) ? result : [result]; // Certificar-se de que data é sempre um array
            setPaginacao(false);
        } else {
            const result = await useService.getByCnpjAndTipo(query, 'LP');
            data = Array.isArray(result) ? result : [result]; // Certificar-se de que data é sempre um array
            setPaginacao(false);
        }
        ...
        <Template loading={loading}>
            ...
            <div>
                <section className='flex items-center justify-between my-5'>
                    <div className='flex'>
                        <h1 className="text-left text-2xl font-semibold text-gray-800">Certificados Lucro Presumido</h1>
                    </div>
                </section>
                ...
                <div className='flex items-center justify-between space-x-4 mb-4'>
                    ...
                    <div className='flex space-x-4 items-center'>
                        ...
                        <InputPJModal open={open} setOpen={setOpen} onAddCertificate={handleAddCertificate} tipoCertificado='LP'/>
                    </div>
                </div>
