...
public interface CertificadoPJRepository extends JpaRepository<CertificadoPJ, String> {
    ...
    @Query("SELECT c FROM CertificadoPJ c WHERE c.tipoCertificado = :tipo ORDER BY c.dataVencimento ASC")
    List<CertificadoPJ> findAllOrderByDataVencimentoAscAndTipoCertificado(@Param("tipo") TipoCertificado tipo, Pageable pageable);

    @Query("SELECT COUNT(c) FROM CertificadoPJ c where c.tipoCertificado = :tipo")
    long countByTipo(@Param("tipo") TipoCertificado tipo);
